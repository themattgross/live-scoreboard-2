...
import { Observable, interval, Subscription, BehaviorSubject, of } from 'rxjs';
      ...
      switchMap(date => {
        ...
        if (isToday) {
          ...
          return interval(1 * 60 * 1000).pipe(
            startWith(0),
            switchMap(() => this.scoresService.getMlb(dateStr)),
            tap(() => console.log(`MLB Data refreshed for ${dateStr}.`))
          );
        } else {
          // Fetch only once for past/future dates
          return this.scoresService.getMlb(dateStr).pipe(
            tap(() => console.log(`MLB Data fetched for ${dateStr}.`))
          );
        }
      })
    );
